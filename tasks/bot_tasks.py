...
def process_video_task(
    ...
    if not acquire_lock(lock_key):
        ...
        return
    try:
        asyncio.run(_process_video_task_async(chat_id, url, status_message_id, format_height))
    finally:
...
async def _process_video_task_async(
    ...
    try:
        ...
        video_path = await download_video(url, chat_id, format_height=format_height, progress_hook=progress_hook)
        if video_path:
